window.SITE_DATA = {
  "updatedAt": "2026-06-16 19:04",
  "dashboard": {
    "headline": "2026 世界杯预测、复盘与专题研究中枢",
    "subtitle": "把赛前判断、赛后校准、专题模型和趋势信号放进同一套静态结构里，方便日更追踪。",
    "today": {
      "title": "今日主推",
      "match": "伊拉克 vs 挪威",
      "time": "2026-06-16",
      "score": "挪威将以2-3球的差距完胜伊拉克——这是本届世界杯小组赛中最无悬念的对决之一。",
      "confidence": "高",
      "probabilities": [
        {
          "label": "伊拉克胜",
          "value": 5,
          "tone": "green"
        },
        {
          "label": "平局",
          "value": 13,
          "tone": "gold"
        },
        {
          "label": "挪威胜",
          "value": 82,
          "tone": "red"
        }
      ],
      "reasons": [
        "哈兰德是\"降维打击\"：当今世界最顶级的中锋面对中东联赛的中卫组合，这不是\"实力差距\"而是\"维度差距\"。哈兰德在英超面对范戴克/迪亚斯等世界级中卫都能持续进球，面对苏拉卡/塔赫辛完全可能上演帽子戏法。伊拉克没有\"防守哈兰德的方案\"——只有\"祈祷哈兰德状态不好\"。",
        "厄德高 vs 伊克巴尔是\"阶层差距\"：阿森纳队长、英超顶级组织核心 vs 乌得勒支创意型中场。厄德高的传球范围、空间洞察力和大赛经验远非伊克巴尔能比。伊拉克的中场将被全面压制，预计控球率仅30-35%。",
        "大赛折扣差距是\"隐性鸿沟\"：伊拉克 ×0.72 vs 挪威 ×0.88，这意味着在世界杯舞台上，伊拉克的实际发挥可能比热身赛低28%，而挪威仅低12%。这16%的差距在\"强vs弱\"的对决中会被放大——强者越强、弱者越弱。"
      ],
      "articleId": "prediction-iraq-norway"
    },
    "reviewCards": [
      {
        "label": "最新复盘",
        "match": "瑞典 vs 突尼斯",
        "result": "瑞典 vs 突尼斯",
        "note": "瑞典5-1大胜突尼斯，是今日5场比赛中预测方向唯一完全正确的一场（胜负判断命中）。虽然比分差距巨大（预测2-1，实际5-1），但\"瑞典胜\"的核心判断是准确的，这也是本场获得6.5/10最高评分的主要原因。",
        "articleId": "review-sweden-tunisia"
      },
      {
        "label": "最新复盘",
        "match": "伊朗 vs 新西兰",
        "result": "伊朗 vs 新西兰",
        "note": "伊朗2-2战平新西兰，又一场\"弱队进攻超预期\"的比赛。Elijah Just的梅开二度（7'和54'）是本场最大惊喜，模型完全没有预测到新西兰能够打入2球，更不用说有球员能梅开二度。这再次印证了世界杯小组赛中\"弱队进攻爆发\"的系统性倾向。",
        "articleId": "review-iran-new-zealand"
      }
    ],
    "metrics": [
      {
        "label": "站内文章",
        "value": "49",
        "note": "四大板块统一索引，按时间倒序更新"
      },
      {
        "label": "已复盘场次",
        "value": "15",
        "note": "复盘样本扩大后，模型判断更可校准"
      },
      {
        "label": "胜平负命中",
        "value": "27%",
        "note": "4/15，先看方向稳定性"
      },
      {
        "label": "复盘评分",
        "value": "5.4",
        "note": "根据复盘文中的自评分动态更新"
      }
    ],
    "model": [
      {
        "label": "胜平负命中",
        "value": 4,
        "total": 15,
        "tone": "red"
      },
      {
        "label": "精确比分命中",
        "value": 0,
        "total": 15,
        "tone": "red"
      },
      {
        "label": "备选比分覆盖",
        "value": 2,
        "total": 15,
        "tone": "red"
      },
      {
        "label": "大小球命中",
        "value": 5,
        "total": 15,
        "tone": "red"
      }
    ],
    "signals": [
      {
        "label": "今日主推",
        "value": "伊拉克 vs 挪威",
        "note": "挪威将以2-3球的差距完胜伊拉克——这是本届世界杯小组赛中最无悬念的对决之一。"
      },
      {
        "label": "最新专题",
        "value": "2026 FIFA World Cup 战报 & 预测",
        "note": "- 时间：2026年6月11日 – 7月19日 - 主办国：美国、加拿大、墨西哥（三国联办，史上首次） - 赛制：首次扩军至48队（12组×4队），每组前2 + 8个最佳第三名晋级32强 - 总场次：104场（从64场扩增） - 截至6…"
      },
      {
        "label": "最新复盘",
        "value": "瑞典 vs 突尼斯",
        "note": "瑞典5-1大胜突尼斯，是今日5场比赛中预测方向唯一完全正确的一场（胜负判断命中）。虽然比分差距巨大（预测2-1，实际5-1），但\"瑞典胜\"的核心判断是准确的，这也是本场获得6.5/10最高评分的主要原因。"
      },
      {
        "label": "预测储备",
        "value": "4 场",
        "note": "已提前写入后续赛程的赛前预测"
      }
    ],
    "workflow": [
      "赛前：同步新预测，提取比分、概率、置信度和关键变量。",
      "赛后：新增复盘，回填命中情况、偏差根因和模型评分。",
      "专题：沉淀魔咒、进球趋势、大比分模型和赛程观察。",
      "仪表盘：聚合最新文章、模型战绩和当天重点信号。"
    ]
  },
  "articles": [
    {
      "id": "prediction-uzbekistan-colombia",
      "type": "prediction",
      "section": "赛前预测",
      "title": "乌兹别克斯坦 vs 哥伦比亚 — 2026世界杯 K组 第1轮 预测报告",
      "date": "2026-06-17",
      "summary": "哥伦比亚将以2-0或1-0取胜，乌兹别克斯坦几乎不可能创造奇迹。但乌兹别克斯坦的防守可能比xG数据暗示的更顽强——总比分可能在3球以内。",
      "file": "prediction-uzbekistan-colombia.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "哥伦比亚将以2-0或1-0取胜，乌兹别克斯坦几乎不可能创造奇迹。但乌兹别克斯坦的防守可能比xG数据暗示的更顽强——总比分可能在3球以内。"
        },
        {
          "label": "胜平负",
          "value": "72/18/10"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "高"
        }
      ],
      "related": [
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "乌兹别克斯坦 vs 哥伦比亚"
    },
    {
      "id": "prediction-ghana-panama",
      "type": "prediction",
      "section": "赛前预测",
      "title": "加纳 vs 巴拿马 — 2026世界杯 L组 第1轮 预测报告",
      "date": "2026-06-17",
      "summary": "这是一场几乎无法预测的比赛——高概率是一场沉闷的平局（36%）。如果有胜者，信号指向巴拿马。但置信度极低，这意味着任何方向都是不安全的。预测比分 1-1。",
      "file": "prediction-ghana-panama.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "这是一场几乎无法预测的比赛——高概率是一场沉闷的平局（36%）。如果有胜者，信号指向巴拿马。但置信度极低，这意味着任何方向都是不安全的。预测比分 1-1。"
        },
        {
          "label": "胜平负",
          "value": "30/36/34"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "极低"
        }
      ],
      "related": [
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "加纳 vs 巴拿马"
    },
    {
      "id": "prediction-england-croatia",
      "type": "prediction",
      "section": "赛前预测",
      "title": "英格兰 vs 克罗地亚 — 2026世界杯 L组 第1轮 预测报告",
      "date": "2026-06-17",
      "summary": "英格兰将以1球优势小胜——但克罗地亚的\"不死\"属性和利瓦科维奇的大赛超人模式让任何预测都不安全。预测比分 2-1，双方都有进球。",
      "file": "prediction-england-croatia.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "英格兰将以1球优势小胜——但克罗地亚的\"不死\"属性和利瓦科维奇的大赛超人模式让任何预测都不安全。预测比分 2-1，双方都有进球。"
        },
        {
          "label": "胜平负",
          "value": "52/26/22"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "中等"
        }
      ],
      "related": [
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "英格兰 vs 克罗地亚"
    },
    {
      "id": "prediction-portugal-dr-congo",
      "type": "prediction",
      "section": "赛前预测",
      "title": "葡萄牙 vs 民主刚果 — 2026世界杯 K组 第1轮 预测报告",
      "date": "2026-06-17",
      "summary": "葡萄牙将以3-0或2-0的大比分取得开门红，民主刚果很难取得进球。",
      "file": "prediction-portugal-dr-congo.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "葡萄牙将以3-0或2-0的大比分取得开门红，民主刚果很难取得进球。"
        },
        {
          "label": "胜平负",
          "value": "73/17/10"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "高"
        }
      ],
      "related": [
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "葡萄牙 vs 民主刚果"
    },
    {
      "id": "prediction-iraq-norway",
      "type": "prediction",
      "section": "赛前预测",
      "title": "2026世界杯预测：伊拉克 vs 挪威",
      "date": "2026-06-16",
      "summary": "挪威将以2-3球的差距完胜伊拉克——这是本届世界杯小组赛中最无悬念的对决之一。",
      "file": "prediction-iraq-norway.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "挪威将以2-3球的差距完胜伊拉克——这是本届世界杯小组赛中最无悬念的对决之一。"
        },
        {
          "label": "胜平负",
          "value": "5/13/82"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "高"
        }
      ],
      "related": [
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "伊拉克 vs 挪威"
    },
    {
      "id": "prediction-austria-jordan",
      "type": "prediction",
      "section": "赛前预测",
      "title": "2026世界杯预测：奥地利 vs 约旦",
      "date": "2026-06-16",
      "summary": "奥地利将以1-2球的差距取胜，2-0是最可能的结果。约旦的阿拉伯杯经验让他们不会完全崩溃，但实力差距终将决定比赛走向。",
      "file": "prediction-austria-jordan.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "奥地利 2-0 约旦（~16%）"
        },
        {
          "label": "胜平负",
          "value": "70/20/10"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "中高"
        }
      ],
      "related": [
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "奥地利 vs 约旦"
    },
    {
      "id": "prediction-france-senegal",
      "type": "prediction",
      "section": "赛前预测",
      "title": "2026世界杯预测 · 法国 vs 塞内加尔",
      "date": "2026-06-16",
      "summary": "法国将以1-2球优势取胜，但塞内加尔有能力制造麻烦——预计比分 2-1，双方都有进球。",
      "file": "prediction-france-senegal.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "法国将以1-2球优势取胜，但塞内加尔有能力制造麻烦——预计比分 2-1，双方都有进球。"
        },
        {
          "label": "胜平负",
          "value": "63/22/15"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "高"
        }
      ],
      "related": [
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "法国 vs 塞内加尔"
    },
    {
      "id": "prediction-argentina-algeria",
      "type": "prediction",
      "section": "赛前预测",
      "title": "🇦🇷 阿根廷 vs 阿尔及利亚 🇩🇿 — 2026世界杯 J组 第1轮 预测报告",
      "date": "2026-06-16",
      "summary": "阿根廷 2-0 阿尔及利亚",
      "file": "prediction-argentina-algeria.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "阿根廷 2-0 阿尔及利亚"
        },
        {
          "label": "胜平负",
          "value": "65/23/12"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "中"
        }
      ],
      "related": [
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "阿根廷 vs 阿尔及利亚"
    },
    {
      "id": "feature-daily-brief-2026-06-16",
      "type": "feature",
      "section": "专题研究",
      "title": "2026 FIFA World Cup 战报 & 预测",
      "date": "2026-06-16",
      "summary": "- 时间：2026年6月11日 – 7月19日 - 主办国：美国、加拿大、墨西哥（三国联办，史上首次） - 赛制：首次扩军至48队（12组×4队），每组前2 + 8个最佳第三名晋级32强 - 总场次：104场（从64场扩增） - 截至6…",
      "file": "feature-daily-brief-2026-06-16.md",
      "tags": [
        "进球趋势",
        "大比分"
      ],
      "stats": [
        {
          "label": ":--:",
          "value": ":--"
        },
        {
          "label": "1",
          "value": "墨西哥（H）"
        },
        {
          "label": "2",
          "value": "韩国"
        },
        {
          "label": "3",
          "value": "捷克"
        }
      ],
      "related": [
        "feature-coach-analysis-groups-il",
        "feature-coach-analysis-groups-eh",
        "feature-coach-analysis-groups-ad"
      ],
      "subtype": "赛程观察",
      "match": "墨西哥 vs 南非"
    },
    {
      "id": "feature-coach-analysis-groups-il",
      "type": "feature",
      "section": "专题研究",
      "title": "2026年世界杯主教练执教水平深度对比分析",
      "date": "2026-06-16",
      "summary": "I组赛程：6月15日（法国vs塞内加尔、伊拉克vs挪威），6月20日（法国vs伊拉克、挪威vs塞内加尔），6月24日（挪威vs法国、塞内加尔vs伊拉克）",
      "file": "feature-coach-analysis-groups-il.md",
      "tags": [
        "魔咒",
        "进球趋势",
        "小组赛",
        "大比分"
      ],
      "stats": [
        {
          "label": "I组",
          "value": "法国"
        },
        {
          "label": "J组",
          "value": "阿根廷"
        },
        {
          "label": "K组",
          "value": "葡萄牙"
        },
        {
          "label": "L组",
          "value": "󠁧󠁢󠁥󠁮󠁧󠁿 英格兰"
        }
      ],
      "related": [
        "feature-daily-brief-2026-06-16",
        "feature-coach-analysis-groups-eh",
        "feature-coach-analysis-groups-ad"
      ],
      "subtype": "教练研究"
    },
    {
      "id": "feature-coach-analysis-groups-eh",
      "type": "feature",
      "section": "专题研究",
      "title": "2026 世界杯主教练执教水平深度对比分析",
      "date": "2026-06-16",
      "summary": "近期成绩与球队状态",
      "file": "feature-coach-analysis-groups-eh.md",
      "tags": [
        "进球趋势",
        "小组赛",
        "大比分"
      ],
      "stats": [
        {
          "label": "E组",
          "value": "德国"
        },
        {
          "label": "F组",
          "value": "荷兰"
        },
        {
          "label": "G组",
          "value": "比利时"
        },
        {
          "label": "H组",
          "value": "西班牙"
        }
      ],
      "related": [
        "feature-daily-brief-2026-06-16",
        "feature-coach-analysis-groups-il",
        "feature-coach-analysis-groups-ad"
      ],
      "subtype": "教练研究"
    },
    {
      "id": "feature-coach-analysis-groups-ad",
      "type": "feature",
      "section": "专题研究",
      "title": "2026 世界杯主教练执教水平深度对比分析",
      "date": "2026-06-16",
      "summary": "执教水平评分：7.0/10",
      "file": "feature-coach-analysis-groups-ad.md",
      "tags": [
        "魔咒",
        "进球趋势",
        "小组赛",
        "大比分"
      ],
      "stats": [
        {
          "label": "A组",
          "value": "墨西哥"
        },
        {
          "label": "B组",
          "value": "加拿大"
        },
        {
          "label": "C组",
          "value": "巴西"
        },
        {
          "label": "D组",
          "value": "美国"
        }
      ],
      "related": [
        "feature-daily-brief-2026-06-16",
        "feature-coach-analysis-groups-il",
        "feature-coach-analysis-groups-eh"
      ],
      "subtype": "教练研究"
    },
    {
      "id": "feature-wc2026-blowout-prediction",
      "type": "feature",
      "section": "专题研究",
      "title": "2026 世界杯小组赛「大比分高概率场次」预测分析",
      "date": "2026-06-16",
      "summary": "2026 世界杯小组赛 72 场中，极高风险（≥70% 概率）的大比分场次约 6-7 场，高风险（40-70%）约 8-10 场。四支首秀球队（库拉索、佛得角、约旦、乌兹别克斯坦）+ 两支久违回归球队（海地、伊拉克）是主要\"受害者\"。",
      "file": "feature-wc2026-blowout-prediction.md",
      "tags": [
        "小组赛",
        "大比分"
      ],
      "stats": [
        {
          "label": "1",
          "value": "FIFA 排名差 ≥ 30"
        },
        {
          "label": "2",
          "value": "弱队为世界杯新军"
        },
        {
          "label": "3",
          "value": "弱队久违参赛 / 经验严重不足"
        },
        {
          "label": "4",
          "value": "亚洲/非洲防线 vs 欧洲/南美顶级锋线"
        }
      ],
      "related": [
        "feature-daily-brief-2026-06-16",
        "feature-coach-analysis-groups-il",
        "feature-coach-analysis-groups-eh"
      ],
      "subtype": "大比分模型",
      "match": "亚洲/非洲防线 vs 欧洲/南美顶级锋线"
    },
    {
      "id": "feature-blowout-analysis",
      "type": "feature",
      "section": "专题研究",
      "title": "2018 & 2022 世界杯「大比分比赛」特征深度分析报告",
      "date": "2026-06-16",
      "summary": "大比分比赛的三大支柱：实力断崖差 × 早期进球 × 心理崩溃",
      "file": "feature-blowout-analysis.md",
      "tags": [
        "魔咒",
        "进球趋势",
        "小组赛",
        "大比分"
      ],
      "stats": [
        {
          "label": "1",
          "value": "小组赛 A 组"
        },
        {
          "label": "2",
          "value": "小组赛 G 组"
        },
        {
          "label": "3",
          "value": "小组赛 G 组"
        },
        {
          "label": "4",
          "value": "小组赛 B 组"
        }
      ],
      "related": [
        "feature-daily-brief-2026-06-16",
        "feature-coach-analysis-groups-il",
        "feature-coach-analysis-groups-eh"
      ],
      "subtype": "强弱差模型",
      "match": "俄罗斯 vs 沙特阿拉伯"
    },
    {
      "id": "review-sweden-tunisia",
      "type": "review",
      "section": "赛后复盘",
      "title": "复盘：瑞典 vs 突尼斯（2026-06-15）",
      "date": "2026-06-15",
      "summary": "瑞典5-1大胜突尼斯，是今日5场比赛中预测方向唯一完全正确的一场（胜负判断命中）。虽然比分差距巨大（预测2-1，实际5-1），但\"瑞典胜\"的核心判断是准确的，这也是本场获得6.5/10最高评分的主要原因。",
      "file": "review-sweden-tunisia.md",
      "tags": [
        "赛后复盘",
        "xG"
      ],
      "stats": [
        {
          "label": "实际比分",
          "value": "瑞典 vs 突尼斯"
        },
        {
          "label": "胜平负",
          "value": "瑞典胜"
        },
        {
          "label": "大小球",
          "value": "待补充"
        },
        {
          "label": "复盘评分",
          "value": "6.5/10"
        }
      ],
      "related": [
        "prediction-sweden-tunisia",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "瑞典 vs 突尼斯"
    },
    {
      "id": "review-iran-new-zealand",
      "type": "review",
      "section": "赛后复盘",
      "title": "复盘：伊朗 vs 新西兰（2026-06-15）",
      "date": "2026-06-15",
      "summary": "伊朗2-2战平新西兰，又一场\"弱队进攻超预期\"的比赛。Elijah Just的梅开二度（7'和54'）是本场最大惊喜，模型完全没有预测到新西兰能够打入2球，更不用说有球员能梅开二度。这再次印证了世界杯小组赛中\"弱队进攻爆发\"的系统性倾向。",
      "file": "review-iran-new-zealand.md",
      "tags": [
        "赛后复盘",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "实际比分",
          "value": "伊朗 vs 新西兰"
        },
        {
          "label": "胜平负",
          "value": "平局"
        },
        {
          "label": "大小球",
          "value": "待补充"
        },
        {
          "label": "复盘评分",
          "value": "4.5/10"
        }
      ],
      "related": [
        "prediction-iran-new-zealand",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "伊朗 vs 新西兰"
    },
    {
      "id": "review-saudi-arabia-uruguay",
      "type": "review",
      "section": "赛后复盘",
      "title": "复盘：沙特阿拉伯 vs 乌拉圭（2026-06-15）",
      "date": "2026-06-15",
      "summary": "沙特阿拉伯1-1战平乌拉圭，又一场高置信度预测的翻车。本场比赛与西班牙vs佛得角存在高度相似性——弱队门将超神（Al-Owais 8次扑救）、强队进攻效率低下、先入球方为弱队。这暗示模型可能存在系统性缺陷：在世界杯小组赛中，对弱队门将爆…",
      "file": "review-saudi-arabia-uruguay.md",
      "tags": [
        "赛后复盘",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "实际比分",
          "value": "沙特阿拉伯 vs 乌拉圭"
        },
        {
          "label": "胜平负",
          "value": "平局"
        },
        {
          "label": "大小球",
          "value": "待补充"
        },
        {
          "label": "复盘评分",
          "value": "4.5/10"
        }
      ],
      "related": [
        "prediction-saudi-arabia-uruguay",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "沙特阿拉伯 vs 乌拉圭"
    },
    {
      "id": "review-belgium-egypt",
      "type": "review",
      "section": "赛后复盘",
      "title": "复盘：比利时 vs 埃及（2026-06-15）",
      "date": "2026-06-15",
      "summary": "比利时1-1战平埃及，预测方向正确识别了双方均有进球的可能，但在胜负判断和比分预测上均未命中。相比西班牙vs佛得角的系统性失败，本场更像是\"差了一步\"——控球率、xG等指标均在合理范围内，只是比赛的关键节点（Ashour先入球、乌龙球）…",
      "file": "review-belgium-egypt.md",
      "tags": [
        "赛后复盘",
        "xG"
      ],
      "stats": [
        {
          "label": "实际比分",
          "value": "比利时 vs 埃及"
        },
        {
          "label": "胜平负",
          "value": "平局"
        },
        {
          "label": "大小球",
          "value": "待补充"
        },
        {
          "label": "复盘评分",
          "value": "5.5/10"
        }
      ],
      "related": [
        "prediction-belgium-egypt",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "比利时 vs 埃及"
    },
    {
      "id": "review-spain-cabo-verde",
      "type": "review",
      "section": "赛后复盘",
      "title": "复盘：西班牙 vs 佛得角（2026-06-15）",
      "date": "2026-06-15",
      "summary": "本场比赛是2026世界杯开赛以来的最大冷门之一，佛得角以0-0逼平西班牙，完全颠覆了模型预测。从预测角度看，这是一次系统性的失败——不仅比分错误，连最基本的方向判断（西班牙胜）都未命中。核心问题在于模型过于依赖球队纸面实力差异，而忽视了…",
      "file": "review-spain-cabo-verde.md",
      "tags": [
        "赛后复盘",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "实际比分",
          "value": "西班牙 vs 佛得角"
        },
        {
          "label": "胜平负",
          "value": "平局"
        },
        {
          "label": "大小球",
          "value": "待补充"
        },
        {
          "label": "复盘评分",
          "value": "3.0/10"
        }
      ],
      "related": [
        "prediction-spain-cabo-verde",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "西班牙 vs 佛得角"
    },
    {
      "id": "prediction-sweden-tunisia",
      "type": "prediction",
      "section": "赛前预测",
      "title": "🇸🇪 瑞典 vs 突尼斯 🇹🇳 - 2026世界杯预测报告",
      "date": "2026-06-15",
      "summary": "最可能比分：瑞典 2-1 突尼斯（概率 ~17%）",
      "file": "prediction-sweden-tunisia.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "瑞典 2-1 突尼斯"
        },
        {
          "label": "胜平负",
          "value": "50/28/22"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": ""
        }
      ],
      "related": [
        "review-sweden-tunisia",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "瑞典 vs 突尼斯"
    },
    {
      "id": "prediction-iran-new-zealand",
      "type": "prediction",
      "section": "赛前预测",
      "title": "伊朗 vs 新西兰 · 2026世界杯预测",
      "date": "2026-06-15",
      "summary": "预测逻辑： FIFA排名差距64位 + 伊朗对Tier 3-4对手全胜零失球 + Taremi个人能力碾压 → 2条强信号指向伊朗胜。Azmoun缺阵是负面因素但不足以翻转判断。Chris Wood是新西兰唯一的变量——如果他的膝伤恢复…",
      "file": "prediction-iran-new-zealand.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "待补充"
        },
        {
          "label": "胜平负",
          "value": "53/29/18"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "中"
        }
      ],
      "related": [
        "review-iran-new-zealand",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "伊朗 vs 新西兰"
    },
    {
      "id": "prediction-saudi-arabia-uruguay",
      "type": "prediction",
      "section": "赛前预测",
      "title": "沙特阿拉伯 vs 乌拉圭 · 2026世界杯预测",
      "date": "2026-06-15",
      "summary": "预测逻辑： FIFA排名差距46位 + 沙特近10场对强队0胜 + 临阵换帅仅2个月 → 3条强信号一致指向乌拉圭胜。迈阿密高温是唯一利好沙特的因素，但不足以翻转52%的胜率差值。",
      "file": "prediction-saudi-arabia-uruguay.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "待补充"
        },
        {
          "label": "胜平负",
          "value": "12/24/64"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "高"
        }
      ],
      "related": [
        "review-saudi-arabia-uruguay",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "沙特阿拉伯 vs 乌拉圭"
    },
    {
      "id": "prediction-belgium-egypt",
      "type": "prediction",
      "section": "赛前预测",
      "title": "2026世界杯预测 · 比利时 vs 埃及（2026-06-15）",
      "date": "2026-06-15",
      "summary": "比利时将以 1-2 球的差距小胜埃及——这场比赛的\"小球\"属性更强，预计不会出现 3 球以上的大比分。",
      "file": "prediction-belgium-egypt.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "比利时 2-1 埃及"
        },
        {
          "label": "胜平负",
          "value": "55/27/18"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "中"
        }
      ],
      "related": [
        "review-belgium-egypt",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "比利时 vs 埃及"
    },
    {
      "id": "prediction-spain-cabo-verde",
      "type": "prediction",
      "section": "赛前预测",
      "title": "2026世界杯预测 · 西班牙 vs 佛得角（2026-06-15）",
      "date": "2026-06-15",
      "summary": "西班牙将以 2-3 球的差距轻松取胜，但不会是一场\"屠戮\"——德拉富恩特可能\"按部就班\"取胜后让主力休息。",
      "file": "prediction-spain-cabo-verde.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "西班牙 3-0 佛得角"
        },
        {
          "label": "胜平负",
          "value": "80/14/6"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "高"
        }
      ],
      "related": [
        "review-spain-cabo-verde",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "西班牙 vs 佛得角"
    },
    {
      "id": "review-ivory-coast-ecuador",
      "type": "review",
      "section": "赛后复盘",
      "title": "科特迪瓦 vs 厄瓜多尔 · 复盘笔记",
      "date": "2026-06-14",
      "summary": "方向： 命中（科特迪瓦胜，AI 概率 51%）",
      "file": "review-ivory-coast-ecuador.md",
      "tags": [
        "赛后复盘",
        "xG"
      ],
      "stats": [
        {
          "label": "实际比分",
          "value": "科特迪瓦 1-0 厄瓜多尔"
        },
        {
          "label": "胜平负",
          "value": "待补充"
        },
        {
          "label": "大小球",
          "value": "待补充"
        },
        {
          "label": "复盘评分",
          "value": "6.5/10"
        }
      ],
      "related": [
        "prediction-ivory-coast-ecuador",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "科特迪瓦 vs 厄瓜多尔"
    },
    {
      "id": "review-netherlands-japan",
      "type": "review",
      "section": "赛后复盘",
      "title": "2026 世界杯预测复盘报告",
      "date": "2026-06-14",
      "summary": "总体评价：预测报告在\"球员定位\"和\"战术框架\"层面表现良好——范戴克、赫拉芬贝赫、Ueda的角色预判基本正确，4-2-3-1 vs 3-4-2-1 的战术框架也准确。",
      "file": "review-netherlands-japan.md",
      "tags": [
        "赛后复盘",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "实际比分",
          "value": "荷兰 2-2 日本 （平局）"
        },
        {
          "label": "胜平负",
          "value": "待补充"
        },
        {
          "label": "大小球",
          "value": "待补充"
        },
        {
          "label": "复盘评分",
          "value": "4.0 / 10"
        }
      ],
      "related": [
        "prediction-netherlands-japan",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "荷兰 vs 日本"
    },
    {
      "id": "review-haiti-scotland",
      "type": "review",
      "section": "赛后复盘",
      "title": "2026 世界杯预测复盘报告",
      "date": "2026-06-14",
      "summary": "总体评价：预测报告在\"赛果方向\"上完全正确，在\"具体比分\"上方向对但差 1 球。苏格兰胜的概率预测合理，FIFA 排名差距、苏格兰整体实力、海地首次参赛压力等关键判断均正确，麦金作为中场核心的威胁判断也得到验证（他打入了唯一进球）。",
      "file": "review-haiti-scotland.md",
      "tags": [
        "赛后复盘",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "实际比分",
          "value": "海地 0-1 苏格兰 󠁧󠁢󠁳󠁣󠁴󠁿（麦金 28'）"
        },
        {
          "label": "胜平负",
          "value": "待补充"
        },
        {
          "label": "大小球",
          "value": "待补充"
        },
        {
          "label": "复盘评分",
          "value": "7.0 / 10"
        }
      ],
      "related": [
        "prediction-haiti-scotland",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "海地 vs 苏格兰"
    },
    {
      "id": "review-germany-curacao",
      "type": "review",
      "section": "赛后复盘",
      "title": "2026 世界杯预测复盘报告",
      "date": "2026-06-14",
      "summary": "总体评价：预测报告在\"概率方向\"上完全正确（德国大胜），但在\"具体比分\"上严重偏差。赛果方向（德国胜）正确、胜平负概率分布合理、纳格尔斯曼战术灵活型判断准确、诺伊尔替代失球风险也得到验证。这些都是本场预测的亮点。",
      "file": "review-germany-curacao.md",
      "tags": [
        "赛后复盘",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "实际比分",
          "value": "德国 7-1 库拉索"
        },
        {
          "label": "胜平负",
          "value": "待补充"
        },
        {
          "label": "大小球",
          "value": "待补充"
        },
        {
          "label": "复盘评分",
          "value": "5.5 / 10"
        }
      ],
      "related": [
        "prediction-germany-curacao",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "德国 vs 库拉索"
    },
    {
      "id": "review-australia-turkey",
      "type": "review",
      "section": "赛后复盘",
      "title": "2026 世界杯预测复盘报告",
      "date": "2026-06-14",
      "summary": "总体评价：预测报告在\"赛前信息\"和\"球员覆盖\"层面表现良好——20人/队的球员数据库、xG区间估计、犯规率/纪律指数、门将专项分析都齐全，伊尔迪兹伤情判断、延吉首发概率评估也基本准确。",
      "file": "review-australia-turkey.md",
      "tags": [
        "赛后复盘",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "实际比分",
          "value": "澳大利亚 2-0 土耳其"
        },
        {
          "label": "胜平负",
          "value": "（方向相反）"
        },
        {
          "label": "大小球",
          "value": "待补充"
        },
        {
          "label": "复盘评分",
          "value": "4.5 / 10"
        }
      ],
      "related": [
        "prediction-australia-turkey",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "澳大利亚 vs 土耳其"
    },
    {
      "id": "prediction-ivory-coast-ecuador",
      "type": "prediction",
      "section": "赛前预测",
      "title": "🇨🇮 科特迪瓦 vs 厄瓜多尔 🇪🇨 - 2026世界杯预测报告",
      "date": "2026-06-14",
      "summary": "最可能比分：科特迪瓦 1-1 厄瓜多尔（概率 ~11%）",
      "file": "prediction-ivory-coast-ecuador.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "科特迪瓦 1-1 厄瓜多尔"
        },
        {
          "label": "胜平负",
          "value": "51/21/28"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": ""
        }
      ],
      "related": [
        "review-ivory-coast-ecuador",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "科特迪瓦 vs 厄瓜多尔"
    },
    {
      "id": "prediction-netherlands-japan",
      "type": "prediction",
      "section": "赛前预测",
      "title": "🇳🇱 荷兰 vs 日本 🇯🵰 - 2026世界杯预测报告",
      "date": "2026-06-14",
      "summary": "最可能比分：荷兰 1-2 日本（概率 ~12%）",
      "file": "prediction-netherlands-japan.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "荷兰 1-2 日本"
        },
        {
          "label": "胜平负",
          "value": "35/18/47"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": ""
        }
      ],
      "related": [
        "review-netherlands-japan",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "荷兰 vs 日本"
    },
    {
      "id": "prediction-germany-curacao",
      "type": "prediction",
      "section": "赛前预测",
      "title": "德国 vs 库拉索 - 2026 世界杯预测报告",
      "date": "2026-06-14",
      "summary": "预测： 德国 3-0 库拉索 （置信度 高）",
      "file": "prediction-germany-curacao.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": ""
        },
        {
          "label": "胜平负",
          "value": "待补充"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "中"
        }
      ],
      "related": [
        "review-germany-curacao",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "德国 vs 库拉索"
    },
    {
      "id": "prediction-australia-turkey",
      "type": "prediction",
      "section": "赛前预测",
      "title": "2026世界杯预测报告：澳大利亚 vs 土耳其",
      "date": "2026-06-14",
      "summary": "土耳其将凭借中场技术优势和恰尔汗奥卢的定位球取胜，但比分将非常接近——这是一场\"1球决胜\"的比赛。",
      "file": "prediction-australia-turkey.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "土耳其 1-0 澳大利亚"
        },
        {
          "label": "胜平负",
          "value": "待补充"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "中"
        }
      ],
      "related": [
        "review-australia-turkey",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "澳大利亚 vs 土耳其"
    },
    {
      "id": "prediction-haiti-scotland",
      "type": "prediction",
      "section": "赛前预测",
      "title": "2026世界杯预测报告：海地 vs 苏格兰",
      "date": "2026-06-14",
      "summary": "苏格兰将在时隔28年的世界杯首战中取得胜利。",
      "file": "prediction-haiti-scotland.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "苏格兰 2-0 海地"
        },
        {
          "label": "胜平负",
          "value": "待补充"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "高"
        }
      ],
      "related": [
        "review-haiti-scotland",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "海地 vs 苏格兰"
    },
    {
      "id": "review-brazil-morocco",
      "type": "review",
      "section": "赛后复盘",
      "title": "复盘 · 巴西 vs 摩洛哥（2026-06-13）",
      "date": "2026-06-13",
      "summary": "比分命中 （1-1 16% 命中但非首选） | 大小球命中 | 概率方向部分正确",
      "file": "review-brazil-morocco.md",
      "tags": [
        "赛后复盘",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "实际比分",
          "value": "巴西 1-1 摩洛哥（1-1 16% 命中但非首选）"
        },
        {
          "label": "胜平负",
          "value": "待补充"
        },
        {
          "label": "大小球",
          "value": "部分命中"
        },
        {
          "label": "复盘评分",
          "value": "6.0 / 10"
        }
      ],
      "related": [
        "prediction-brazil-morocco",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "巴西 vs 摩洛哥"
    },
    {
      "id": "review-qatar-switzerland",
      "type": "review",
      "section": "赛后复盘",
      "title": "复盘 · 卡塔尔 vs 瑞士（2026-06-13）",
      "date": "2026-06-13",
      "summary": "总比分命中 | 胜平负命中 | 大小球命中",
      "file": "review-qatar-switzerland.md",
      "tags": [
        "赛后复盘",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "实际比分",
          "value": "卡塔尔 1-1 瑞士"
        },
        {
          "label": "胜平负",
          "value": "命中"
        },
        {
          "label": "大小球",
          "value": "#"
        },
        {
          "label": "复盘评分",
          "value": "5.0 / 10"
        }
      ],
      "related": [
        "prediction-qatar-switzerland",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "卡塔尔 vs 瑞士"
    },
    {
      "id": "review-canada-bosnia-herzegovina",
      "type": "review",
      "section": "赛后复盘",
      "title": "复盘报告：加拿大 vs 波黑（2026-06-13）",
      "date": "2026-06-13",
      "summary": "复盘给分：6.5 / 10",
      "file": "review-canada-bosnia-herzegovina.md",
      "tags": [
        "赛后复盘",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "实际比分",
          "value": "加拿大 1-1 波黑（平局）"
        },
        {
          "label": "胜平负",
          "value": "待补充"
        },
        {
          "label": "大小球",
          "value": "2球 走盘"
        },
        {
          "label": "复盘评分",
          "value": "6.5 / 10"
        }
      ],
      "related": [
        "prediction-canada-bosnia-herzegovina",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "加拿大 vs 波黑"
    },
    {
      "id": "review-usa-paraguay",
      "type": "review",
      "section": "赛后复盘",
      "title": "复盘报告：美国 vs 巴拉圭（2026-06-13）",
      "date": "2026-06-13",
      "summary": "复盘给分：5.5 / 10",
      "file": "review-usa-paraguay.md",
      "tags": [
        "赛后复盘",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "实际比分",
          "value": "美国 4-1 巴拉圭"
        },
        {
          "label": "胜平负",
          "value": "待补充"
        },
        {
          "label": "大小球",
          "value": "5球 大球"
        },
        {
          "label": "复盘评分",
          "value": "5.5 / 10"
        }
      ],
      "related": [
        "prediction-usa-paraguay",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "美国 vs 巴拉圭"
    },
    {
      "id": "prediction-brazil-morocco",
      "type": "prediction",
      "section": "赛前预测",
      "title": "2026世界杯预测：巴西 vs 摩洛哥",
      "date": "2026-06-13",
      "summary": "预测：巴西小胜，比分 1-0，置信度 中高",
      "file": "prediction-brazil-morocco.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "预测：巴西小胜，比分 1-0，置信度 中高"
        },
        {
          "label": "胜平负",
          "value": "55/26/19"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "高（差值≥15%）"
        }
      ],
      "related": [
        "review-brazil-morocco",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "巴西 vs 摩洛哥"
    },
    {
      "id": "prediction-qatar-switzerland",
      "type": "prediction",
      "section": "赛前预测",
      "title": "2026世界杯预测：卡塔尔 vs 瑞士",
      "date": "2026-06-13",
      "summary": "预测：瑞士胜，比分 0-2，置信度 高",
      "file": "prediction-qatar-switzerland.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "预测：瑞士胜，比分 0-2，置信度 高"
        },
        {
          "label": "胜平负",
          "value": "10/18/72"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "高（差值≥15%）"
        }
      ],
      "related": [
        "review-qatar-switzerland",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "卡塔尔 vs 瑞士"
    },
    {
      "id": "prediction-canada-bosnia-herzegovina",
      "type": "prediction",
      "section": "赛前预测",
      "title": "2026世界杯 · 加拿大 vs 波黑 预测报告",
      "date": "2026-06-13",
      "summary": "预测逻辑： 本场是典型的\"防守 > 进攻\"比赛。加拿大拥有主场之利 + 近10场7零封的铁血防守，但进攻端缺失 Alphonso Davies 后创造力不足。波黑是欧洲预选赛的\"平局大王\"（10场7平），近5场正式比赛全部1-1，淘汰意…",
      "file": "prediction-canada-bosnia-herzegovina.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "1-0 加拿大"
        },
        {
          "label": "胜平负",
          "value": "40/34/26"
        },
        {
          "label": "总进球",
          "value": "1-2球（~65%）"
        },
        {
          "label": "置信度",
          "value": "中等"
        }
      ],
      "related": [
        "review-canada-bosnia-herzegovina",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "加拿大 vs 波黑"
    },
    {
      "id": "prediction-usa-paraguay",
      "type": "prediction",
      "section": "赛前预测",
      "title": "美国 vs 巴拉圭 — 2026世界杯 D组 第1轮 预测报告",
      "date": "2026-06-13",
      "summary": "对手强度校准： - 对 T1（前20名）：4 场 1胜3负，进 6 球失 11 球，场均进 1.50 / 失 2.75 - 对 T2（21-50名）：1 场 1胜，进 2 球失 1 球 - 加权评估：面对强队时防线暴露明显问题（场均失…",
      "file": "prediction-usa-paraguay.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "待补充"
        },
        {
          "label": "胜平负",
          "value": "48/30/22"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "中"
        }
      ],
      "related": [
        "review-usa-paraguay",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "美国 vs 巴拉圭"
    },
    {
      "id": "feature-first-five-goals",
      "type": "feature",
      "section": "专题研究",
      "title": "2026世界杯 · 前五轮进球预测分析",
      "date": "2026-06-13",
      "summary": "核心预测：本届世界杯首轮场均 2.3-2.7球，整体走势偏向\"中位偏低\"，不会像2022年那样爆发也不会像2010年那样冰封。前3场7球（2.33球/场）略低于历史前3场均值（9.25球）。",
      "file": "feature-first-five-goals.md",
      "tags": [
        "进球趋势",
        "小组赛",
        "大比分"
      ],
      "stats": [
        {
          "label": "本届已完赛场次",
          "value": "3场"
        },
        {
          "label": "已进球数",
          "value": "7球"
        },
        {
          "label": "场均进球（3场）",
          "value": "2.33球"
        },
        {
          "label": "首轮总进球预测区间",
          "value": "35-41球（等效32队赛制）/ 55-65球（48队）"
        }
      ],
      "related": [
        "feature-daily-brief-2026-06-16",
        "feature-coach-analysis-groups-il",
        "feature-coach-analysis-groups-eh"
      ],
      "subtype": "进球趋势",
      "match": "墨西哥 vs 南非"
    },
    {
      "id": "review-korea-czech",
      "type": "review",
      "section": "赛后复盘",
      "title": "2026 世界杯预测复盘报告",
      "date": "2026-06-12",
      "summary": "总体评价：预测报告在\"战术框架\"和\"赛前基本信息\"层面表现优秀——正确识别了韩国控球主导、捷克定位球反击、金玟哉的防守核心地位和孙兴慜的攻击威胁。裴俊浩缺阵首发的判断也是准确的。",
      "file": "review-korea-czech.md",
      "tags": [
        "赛后复盘",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "实际比分",
          "value": "韩国 2-1 捷克"
        },
        {
          "label": "胜平负",
          "value": "待补充"
        },
        {
          "label": "大小球",
          "value": "（预测小球，实际3球）"
        },
        {
          "label": "复盘评分",
          "value": "6.0 / 10"
        }
      ],
      "related": [
        "prediction-korea-czech",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": "韩国 vs 捷克"
    },
    {
      "id": "prediction-korea-czech",
      "type": "prediction",
      "section": "赛前预测",
      "title": "2026 FIFA 世界杯 · 赛果预测报告",
      "date": "2026-06-12",
      "summary": "预测：韩国 1-1 捷克 | 置信度：中",
      "file": "prediction-korea-czech.md",
      "tags": [
        "赛前预测",
        "xG",
        "世界杯"
      ],
      "stats": [
        {
          "label": "预测比分",
          "value": "韩国 1-1 捷克"
        },
        {
          "label": "胜平负",
          "value": "38/30/32"
        },
        {
          "label": "总进球",
          "value": "待补充"
        },
        {
          "label": "置信度",
          "value": "中"
        }
      ],
      "related": [
        "review-korea-czech",
        "feature-blowout-analysis",
        "feature-over-under-round-one",
        "feature-group-round-one"
      ],
      "match": " vs 巴拉圭（主）"
    },
    {
      "id": "feature-first-three-goals",
      "type": "feature",
      "section": "专题研究",
      "title": "世界杯「前3场」总进球数 · 历史数据",
      "date": "2026-06-12",
      "summary": "2006 德国 ██████████████████ 9球 2010 南非 ████████ 4球 2014 巴西 ██████████████████████ 11球 2018 俄罗斯 ██████████████ 7球 2022 卡…",
      "file": "feature-first-three-goals.md",
      "tags": [
        "进球趋势",
        "小组赛"
      ],
      "stats": [
        {
          "label": "6届平均",
          "value": "7.2球"
        },
        {
          "label": "场均进球",
          "value": "2.39球"
        },
        {
          "label": "最高一届",
          "value": "12球（2022 卡塔尔）"
        },
        {
          "label": "最低一届",
          "value": "4球（2010 南非）"
        }
      ],
      "related": [
        "feature-daily-brief-2026-06-16",
        "feature-coach-analysis-groups-il",
        "feature-coach-analysis-groups-eh"
      ],
      "subtype": "进球趋势",
      "match": "墨西哥 vs 南非"
    },
    {
      "id": "feature-group-round-one",
      "type": "feature",
      "section": "专题研究",
      "title": "2026 FIFA世界杯小组赛第一轮数据分析报告",
      "date": "2026-06-11",
      "summary": "核心发现: 2026年世界杯首次扩军至48支球队，分为12个小组。根据数据分析，E组（西班牙、德国、日本、哥斯达黎加）和I组（法国、挪威、塞内加尔、伊拉克）为\"死亡之组\"，竞争激烈程度最高。",
      "file": "feature-group-round-one.md",
      "tags": [
        "进球趋势",
        "小组赛"
      ],
      "stats": [
        {
          "label": "参赛球队总数",
          "value": "48"
        },
        {
          "label": "小组数量",
          "value": "12"
        },
        {
          "label": "小组赛总场次",
          "value": "72"
        },
        {
          "label": "晋级淘汰赛球队",
          "value": "32"
        }
      ],
      "related": [
        "feature-daily-brief-2026-06-16",
        "feature-coach-analysis-groups-il",
        "feature-coach-analysis-groups-eh"
      ],
      "subtype": "小组赛观察",
      "match": "墨西哥 vs 南非"
    },
    {
      "id": "feature-curses-champion",
      "type": "feature",
      "section": "专题研究",
      "title": "2026世界杯魔咒分析：基于历史规律的冠军预测",
      "date": "2026-06-11",
      "summary": "阿根廷三重魔咒缠身: FIFA第一魔咒 + 卫冕魔咒 + 三冠时间魔咒，2026年基本无夺冠可能",
      "file": "feature-curses-champion.md",
      "tags": [
        "魔咒",
        "小组赛"
      ],
      "stats": [
        {
          "label": "1",
          "value": "阿根廷"
        },
        {
          "label": "2",
          "value": "西班牙"
        },
        {
          "label": "3",
          "value": "法国"
        },
        {
          "label": "4",
          "value": "󠁧󠁢󠁥󠁮󠁧󠁿 英格兰"
        }
      ],
      "related": [
        "feature-daily-brief-2026-06-16",
        "feature-coach-analysis-groups-il",
        "feature-coach-analysis-groups-eh"
      ],
      "subtype": "魔咒与历史规律",
      "match": "西班牙 vs 法国"
    },
    {
      "id": "feature-over-under-round-one",
      "type": "feature",
      "section": "专题研究",
      "title": "2026世界杯小组赛第一轮 · 大小球博彩分析",
      "date": "2026-06-11",
      "summary": "真实赔率数据： - DraftKings: Over +115 / Under -145 - FanDuel: Over +110 / Under -130 - BetMGM: Over +120 / Under -140",
      "file": "feature-over-under-round-one.md",
      "tags": [
        "进球趋势",
        "小组赛",
        "大比分"
      ],
      "stats": [
        {
          "label": "第一轮比赛总数",
          "value": "24"
        },
        {
          "label": "推荐大球 (Over)",
          "value": "11"
        },
        {
          "label": "推荐小球 (Under)",
          "value": "10"
        },
        {
          "label": "均势/观望",
          "value": "3"
        }
      ],
      "related": [
        "feature-daily-brief-2026-06-16",
        "feature-coach-analysis-groups-il",
        "feature-coach-analysis-groups-eh"
      ],
      "subtype": "进球趋势",
      "match": "控球率、传控打法 vs 防守反击"
    }
  ]
};
